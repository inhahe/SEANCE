...
- **MIDI Script** — an algorithmic MIDI generator you program in a tiny expression language. Your program runs once per sample and emits MIDI live with `note(pitch, vel, durSec)`, `noteon` / `noteoff`, `cc(number, value)` and `bend(value)`. Variables you assign **persist across samples**, so you can keep running counters, phases or RNG seeds; statements are separated by `;` or new lines. The program reads the transport (`t`, `beat`, `bar`, `bpm`, `playing`, `sr`, `dt`), the most-recent MIDI-input note (`note` / `vel` / `gate`), 0..16 **Signal input pins** (`s1..sN`), and an optional built-in **shape table** sampled with `shape(pos)`. A node can have **multiple independent MIDI outputs** (1..16, each its own cable) — route emits to output *k* with `out = k`. For example, a four-on-the-floor kick is `ph = beat - floor(beat); (ph < dt*bpm/60) ? note(36, 110, 0.1) : 0`. Double-click the node to edit; the editor has a full language reference. A **Language** dropdown in the editor switches the program between the built-in expression language, **Lua** (a full embedded scripting language — choose per-sample or per-block execution), and a **WebAssembly** module you load from a `.wasm` file (C, Rust, Zig, AssemblyScript, …) — so you can stay no-setup or reach for a real programming language without changing nodes. See [MIDI Script](REFERENCE.md#midi-script-algorithmic-midi) in REFERENCE.md for the full vocabulary.
...
- **Signal Shape (LFO / Envelope)** — one node that covers both LFO and envelope use. Build the shape from a **stack of layers** (each a sine / saw / square / triangle / noise / drawn points / freehand / formula) summed into one contour, using the exact same layer editor as the wavetable instrument — *+ Layer* button, preset menu, and all — with a live "sum of all layers" preview. A fresh node starts with **zero layers** (flat 0 output) so it never sweeps anything until you add a layer. The shape advances at a chosen rate (free or beat-synced), and a per-sample expression turns the curve into the final output (defaults to "pass through"). A separate trigger expression decides when the cycle starts: empty = always running (LFO), or anything that goes high on a note — e.g. `gate` — gives envelope behaviour. Choose **Forever / Once / N times** for what happens at the end of a cycle. You can add 0..16 Signal input pins (s1..sN) and use them inside the expressions, so the node can react to other signals as well as MIDI. A **Manual Trigger** button auditions one-shot shapes without wiring anything. Like MIDI Script, a **Language** dropdown lets the output be computed by the built-in expression language, **Lua** (per-sample, where it transforms the drawn `curve`, or per-block, where it fills the buffer directly), or a **WebAssembly** `.wasm` module. Double-click the node to edit. See [Signal Shape](REFERENCE.md#signal-shape-lfo--envelope) in REFERENCE.md for the full vocabulary.

...
//
// M1 policy (mirrors poly-voice-architecture.md):
//   - note-on   -> first inactive slot, else steal the OLDEST active slot.
...
struct VoiceAllocator {
    ...
    // Choose the slot a new note would use: first inactive, else oldest active.
    // Returns -1 only when there are no slots at all.
    int pickSlot() const {

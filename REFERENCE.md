...
Every failed load is also written to `seance.log` (`MIDI Script "<name>": <message>`), so there's a persistent record even if no UI is showing the error at the time.

...
**Two repos, and what the tag points at.** The development tree has no git remote; the source published to `inhahe/seance` lives in a separate clone that's synced from here and pushed by hand. A git tag has to name a commit GitHub already has, so `release.bat` asks GitHub whether it has the local `HEAD` before pinning the tag to it. When it does — i.e. the source was synced and pushed before releasing — the tag marks the exact commit the binary was built from. When it doesn't, the script prints a note and creates the tag on the default branch instead, so the release still publishes rather than failing with `Release.target_commitish is invalid`. The binary is unaffected either way; only the precision of "which source is this" changes, which is why syncing and pushing before publishing is worth the extra step.

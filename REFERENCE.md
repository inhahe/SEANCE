...
> - The editor no longer mirrors the op amount back into the node param while pinned. `pushWarpAmountsToParams` only overwrites a param's value when the op is **not** pinned (`!hasParamModPin`); a pinned op's `Param::value` is owned by the node slider / cable, so the editor leaves it alone instead of clobbering it on every edit.
> - The editor's **preview and held "Preview" audition render at the live node-param amount**, not the frozen editor amount (`renderEditingFrameLiveCycle`). So dragging the node-graph slider of a pinned morph op is both **visible** in the preview and **audible** in the sustained audition. Because the held audition overrides the synth terrain with a static cycle, a low-rate poll (`pollNodeParamChanges`, 20 Hz while the editor is open) re-ships that cycle whenever a node-param amount changes underneath the editor, so a node-side drag — or a modulation cable, or an undo — tracks live rather than appearing to do nothing.

**Continuous, click-free Preview while dragging (in-place cycle swap + crossfade).** The sustained "Preview" voice captures its single cycle at note-on. The editor re-ships a freshly rendered cycle on every morph/gain edit (`refreshHeldFrameAudition` ← `onLayerChanged`), but a re-shipped cycle used to take effect only when the debounced graph rebuild (~150 ms) recreated the synth processor — so dragging a morph slider updated the audio in audible *steps* rather than smoothly. The synth now updates the **live audition voice's cycle in place** each block when a newer cycle is shipped (`TerrainSynthProcessor::processBlock`, the `wantHeld && heldAuditionActive` branch). A hard pointer swap would still **click**, though: the phase accumulator is mid-cycle, and reading a freshly reshaped table at that phase steps the output (this is exactly why dragging the slider had audible breaks while a modulation **signal** stayed smooth — the signal path warps the terrain *per sample*, never swapping a whole table). So the swap is **crossfaded**: the voice keeps the previous cycle (`Voice::auditionCyclePrev`) and linearly ramps `auditionCycleFade` 0→1 over ~6 ms, blending `prev`→`current` per output sample. A swap arriving mid-fade re-bases the fade on the cycle it was heading to, so `prev` is always a real table and the per-event deltas stay tiny. The Preview now tracks a slider drag continuously **and** without clicks, matching the signal path.

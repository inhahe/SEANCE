...
- **Signal Oscillator.** A monophonic oscillator (sine / saw / square / triangle) driven entirely by **control signals** rather than a MIDI stream: a Pitch input (Hz, read every sample), a Gate input (fires its shared AHDSR envelope on the rising edge, releases on the falling edge), and a Velocity input. It's the first node that reads pitch/gate from Signal pins directly, which makes it the natural inner voice for a **Voice container** — wire VoiceIn's Pitch/Gate/Velocity straight into it — though it works standalone with any Signal source too. See [Voices](#voices-per-voice-polyphony) above.

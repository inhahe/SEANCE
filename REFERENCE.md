...
**Who owns a pinned op's amount.** Pinning an op moves ownership of its amount from the editor's in-frame slider to the **node param** (the slider that appears on the node, plus any cable wired to its pin). The editor's amount slider for that op is therefore **frozen** while pinned (it shows the baked resting value but can't be the live driver). Two pieces make the node param actually take effect:
> - The editor no longer mirrors the frozen op amount back into the node param. `pushWarpAmountsToParams` only overwrites a param's value when the op is **not** pinned (`!hasParamModPin`); a pinned op's `Param::value` is owned by the node slider / cable, so the editor leaves it alone instead of clobbering it on every edit.
...
**Continuous Preview while dragging (in-place cycle swap).** The sustained "Preview" voice captures its single cycle at note-on. The editor re-ships a freshly rendered cycle on every morph/gain edit (`refreshHeldFrameAudition` ← `onLayerChanged`), but a re-shipped cycle used to take effect only when the debounced graph rebuild (~150 ms) recreated the synth processor — so dragging a morph slider updated the audio in audible *steps* rather than smoothly. The synth now updates the **live audition voice's cycle in place** each block when a newer cycle is shipped (`TerrainSynthProcessor::processBlock`, the `wantHeld && heldAuditionActive` branch). Consecutive cycles differ only by the slider delta, so swapping the wavetable the phase accumulator reads is click-free, and the Preview now tracks a slider drag as smoothly as a modulation **signal** does (the signal case was already continuous because the synth rebakes the terrain every block).

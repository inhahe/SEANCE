...
void OscilloscopeComponent::paint(juce::Graphics& g) {
    g.fillAll(juce::Colour(18, 20, 28));
    auto area = getLocalBounds().toFloat().reduced(6);
    ...
    // One pixel of width = one sample. The capture buffer holds 4096
    // samples max, so the effective sample count saturates once the
    // window is dragged that wide.
    ...
    // Draw L and R overlaid. Height already scales because we anchor to
    // area.getCentreY() and use area.getHeight() * 0.45 as the half-span.
